...
im_meristem_tip = subset(x = IM_SPM_cells, idents = "14")
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, subset = subcluster12 == "12_3")
# Subset on a value in the object meta data
...
```{r}
fm_meristem_tip = subset(x = FM_cells,subset = subcluster10 == "10_1")
# Subset on a value in the object meta data
...
```{r}
all_data = c(im_xu16_meristem_tip,im_xu19_meristem_tip,im_xu29_meristem_tip,im_xu32_meristem_tip,sm_xu45_meristem_tip,sm_xu47_meristem_tip,fm_xu43_meristem_tip,fm_xu48_meristem_tip)

```

```{r}
data_stage = c("im","im","im","im","sm","sm","fm","fm")
```
...
```{r}
col_names = c("im_xu16_meristem_tip","im_xu19_meristem_tip","im_xu29_meristem_tip","im_xu32_meristem_tip","sm_xu45_meristem_tip","sm_xu47_meristem_tip","fm_xu43_meristem_tip","fm_xu48_meristem_tip")
```
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_vasculature_bulked.csv", row.names=TRUE)
```
...
contrast_matrix <- makeContrasts(
  "IM vs Other" = im - (fm + sm) / 2,
  "SM vs Other" = sm - (im + fm) / 2,
  "FM vs Other" = fm - (im + sm) / 2,
  levels = des_mat
)
```
...
```
```{r}
write.csv(stats_df, "~/passala/Collaborator_Data/Maize_arabi_Jack_Collab/ear_tassel-data/vasculature_model_1_v_all_low_trim.csv", row.names=TRUE)
```
