...
im_meristem_tip = subset(x = IM_SPM_cells, subset = subcluster6 == "6_0")
...
```{r}
#Subset Seurat object based on identity class, also see ? SubsetData
reg_spm_meristem_tip = subset(x = SPM_cells, idents = "3")
# Subset on a value in the object meta data
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, subset = subcluster12 == "0")
# Subset on a value in the object meta data
...
```{r}
fm_meristem_tip = subset(x = FM_cells,subset = subcluster10 == "7")
# Subset on a value in the object meta data
...
```{r}
all_data = c(im_xu16_meristem_tip,im_xu29_meristem_tip,im_xu32_meristem_tip,reg_spm_xu44_meristem_tip,reg_spm_xu46_meristem_tip,sm_xu45_meristem_tip,sm_xu47_meristem_tip,fm_xu43_meristem_tip,fm_xu48_meristem_tip)

```

```{r}
data_stage = c("im","im","im","spm","spm","sm","sm","fm","fm")
```
...
```{r}
col_names = c("im_xu16_meristem_tip","im_xu29_meristem_tip","im_xu32_meristem_tip","reg_spm_xu44_meristem_tip","reg_spm_xu46_meristem_tip","sm_xu45_meristem_tip","sm_xu47_meristem_tip","fm_xu43_meristem_tip","fm_xu48_meristem_tip")
```
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_meristem_base_bulked.csv", row.names=TRUE)
```
...
```
```{r}
write.csv(stats_df, "~/passala/Collaborator_Data/Maize_arabi_Jack_Collab/ear_tassel-data/meristem_base_model_1_v_all_low_trim.csv", row.names=TRUE)
```
