...
im_meristem_tip = subset(x = IM_SPM_cells, idents = "12")
...
```{r}
#Subset Seurat object based on identity class, also see ? SubsetData
reg_spm_meristem_tip = subset(x = SPM_cells, idents = "7")
# Subset on a value in the object meta data
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, subset = subcluster12 == "3")
# Subset on a value in the object meta data
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_adaxial_periphery_bulked.csv", row.names=TRUE)
```
...
```
```{r}
write.csv(stats_df, "~/passala/Collaborator_Data/Maize_arabi_Jack_Collab/ear_tassel-data/axaxial_meristem_model_1_v_all_low_trim.csv", row.names=TRUE)
```
