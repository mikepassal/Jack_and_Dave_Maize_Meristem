...
im_meristem_tip = subset(x = IM_SPM_cells, idents = "Determinate.Lateral.Organ.(non.epi)")
...
```{r}
#Subset Seurat object based on identity class, also see ? SubsetData
reg_spm_meristem_tip = subset(x = SPM_cells, idents = "Determinate.Lateral.Organ.(non.epi)")
# Subset on a value in the object meta data
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, idents = "Determinate.Lateral.Organ.(non.epi)")
# Subset on a value in the object meta data
...
```{r}
fm_meristem_tip = subset(x = FM_cells,idents = "Determinate.Lateral.Organ.(non.epi)")
# Subset on a value in the object meta data
...
```{r}
br_meristem_tip = subset(x = TSBR_cells,idents ="Determinate.Lateral.Organ.(non.epi)")
# Subset on a value in the object meta data
...
```{r}
all_data = c(im_xu16_meristem_tip,im_xu19_meristem_tip,im_xu29_meristem_tip,im_xu32_meristem_tip,reg_spm_xu44_meristem_tip,reg_spm_xu46_meristem_tip,sm_xu45_meristem_tip,sm_xu47_meristem_tip,fm_xu43_meristem_tip,fm_xu48_meristem_tip,br_xu39_meristem_tip,br_xu40_meristem_tip)

```

```{r}
data_stage = c("im","im","im","im","spm","spm","sm","sm","fm","fm","br","br")
```
...
```{r}
col_names = c("im_xu16_meristem_tip","im_xu19_meristem_tip","im_xu29_meristem_tip","im_xu32_meristem_tip","reg_spm_xu44_meristem_tip","reg_spm_xu46_meristem_tip","sm_xu45_meristem_tip","sm_xu47_meristem_tip","fm_xu43_meristem_tip","fm_xu48_meristem_tip","br_xu39_meristem_tip","br_xu40_meristem_tip")
```
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_Determinate.Lateral.Organ.(non.epi)_bulked_updated_tassel.csv", row.names=TRUE)
```
...
```{r}
write.csv(stats_df, "~/passala/git/Jack_and_Dave_Maize_Meristem/Maize_ear_tassel_updated/Determinate.Lateral.Organ.(non.epi)_model_1_v_all_low_trim.csv", row.names=TRUE)
```
