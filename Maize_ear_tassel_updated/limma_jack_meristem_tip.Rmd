...
im_meristem_tip = subset(x = IM_SPM_cells, idents = "IM.Meristem.Tip.(stem.cell)")
...
```{r}
earlySPM_meristem_tip = subset(x = IM_SPM_cells, idents = "SPM.Meristem.Tip.(stem.cell)")
# Subset on a value in the object meta data

early_spm_xu16_meristem_tip = subset(x = earlySPM_meristem_tip, subset = orig.ident== "xu16" )
early_spm_xu19_meristem_tip = subset(x = earlySPM_meristem_tip, subset = orig.ident== "xu19" )
early_spm_xu29_meristem_tip = subset(x = earlySPM_meristem_tip, subset = orig.ident== "xu29" )
early_spm_xu32_meristem_tip = subset(x = earlySPM_meristem_tip, subset = orig.ident== "xu32" )
```


```{r}
#Subset Seurat object based on identity class, also see ? SubsetData
reg_spm_meristem_tip = subset(x = SPM_cells, idents = "Meristem.Tip.(stem.cell)")
# Subset on a value in the object meta data
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, idents = "Meristem.Tip.(stem.cell)")
# Subset on a value in the object meta data
...
```{r}
fm_meristem_tip = subset(x = FM_cells,idents = "Meristem.Tip.non.epi.(stem.cell)")
# Subset on a value in the object meta data
...
```{r}
br_meristem_tip = subset(x = TSBR_cells,idents ="Meristem.Tip.(stem.cell)")
# Subset on a value in the object meta data
...
```{r}
all_data = c(im_xu16_meristem_tip,im_xu19_meristem_tip,im_xu29_meristem_tip,im_xu32_meristem_tip,reg_spm_xu44_meristem_tip,reg_spm_xu46_meristem_tip,sm_xu45_meristem_tip,sm_xu47_meristem_tip,fm_xu43_meristem_tip,fm_xu48_meristem_tip,br_xu39_meristem_tip,br_xu40_meristem_tip,early_spm_xu16_meristem_tip,early_spm_xu19_meristem_tip,early_spm_xu29_meristem_tip,early_spm_xu32_meristem_tip)

```

```{r}
data_stage = c("im","im","im","im","spm","spm","sm","sm","fm","fm","br","br","espm","espm","espm","espm")
```
...
```{r}
col_names = c("im_xu16_meristem_tip","im_xu19_meristem_tip","im_xu29_meristem_tip","im_xu32_meristem_tip","reg_spm_xu44_meristem_tip","reg_spm_xu46_meristem_tip","sm_xu45_meristem_tip","sm_xu47_meristem_tip","fm_xu43_meristem_tip","fm_xu48_meristem_tip","br_xu39_meristem_tip","br_xu40_meristem_tip","espm_xu_16_meistem_tip","espm_xu19_meristem_tip","espm_x29_meristem_tip","espm_xu32_meristem_tip")
```
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_meristem_tip_bulked_updated_tassel.csv", row.names=TRUE)
```
...
contrast_matrix <- makeContrasts(
  "IM vs Other" = im - (fm + spm + sm + br + espm) / 5,
  "SPM vs Other" = spm - (im + fm + sm + br + espm) / 5,
  "SM vs Other" = sm - (im + fm + spm + br + espm) / 5,
  "FM vs Other" = fm - (im + spm + sm + br + espm) / 5,
  "BR vs Other" = br - (im + spm + sm + fm + espm) / 5,
  "eSPM vs Other" = espm - (im + spm + sm + fm + br) / 5,
  levels = des_mat
)
```
...
```{r}
write.csv(stats_df, "~/passala/git/Jack_and_Dave_Maize_Meristem/Maize_ear_tassel_updated/meristem_tip_model_1_v_all_low_trim.csv", row.names=TRUE)
```
