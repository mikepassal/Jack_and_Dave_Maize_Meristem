...
im_meristem_tip = subset(x = IM_SPM_cells, idents = "Meristem.Epidermis")
...
```{r}
#Subset Seurat object based on identity class, also see ? SubsetData
reg_spm_meristem_tip = subset(x = SPM_cells, idents = "Meristem.Epidermis")
# Subset on a value in the object meta data
...
```{r}
#Subset Seurat object based on identity class, also see ?SubsetData
sm_meristem_tip = subset(x = SM_cells, idents = "Meristem.Epidermis")
# Subset on a value in the object meta data
...
```{r}
fm_meristem_tip = subset(x = FM_cells,idents = c("Meristem.Tip.epi.(stem.cell)","Adaxial.Meristem.Periphery.(epi)"))
# Subset on a value in the object meta data
...
```{r}
br_meristem_tip = subset(x = TSBR_cells,idents ="Meristem.Epidermis")
# Subset on a value in the object meta data
...
```{r}
write.csv(full_merged_psuedobulk, "~/passala/Generated_Tables/Jack_dave_meristem_bulk_csvs/limma_jack_Meristem.Epidermis_bulked_updated_tassel.csv", row.names=TRUE)
```
...
```{r}
write.csv(stats_df, "~/passala/git/Jack_and_Dave_Maize_Meristem/Maize_ear_tassel_updated/Meristem.Epidermis_model_1_v_all_low_trim.csv", row.names=TRUE)
```
